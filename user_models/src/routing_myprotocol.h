...
//Myprotocol Packet Types
 ...
 #define MYPROTOCOL_RREP        2   /* route reply packet type(when a node finds itself congested 
														 ...
														 of time, it reports to sink node.)*/ 
														 
//Format of route request packet(RRP)
typedef struct
{
	UInt8 type;             //type for route request packet 
	NodeAddress address;	//relay node's address
	UInt8 hopCount;         //hop number from relay node to destination node
	UInt32 sequenceNumber;	//sequence number of route request packet
} MyprotocolRreqPacket;
